...
const Minting = ({ wallet, chain, stats, reload, woolBalance }) => {
  ...
  const ethCost = () => {
    return (
      utils.formatUnits(
        utils.parseUnits(MINT_PRICE, "gwei").mul(BigNumber.from(amount)),
        "gwei"
      ) + " AVAX"
    );
  };
  ...
  const tractorCost = () => {
    return (
      utils.formatUnits(
        utils.parseUnits(TRACTOR_PRICE, "gwei").mul(BigNumber.from(amount)),
        "gwei"
      ) + " TRACTOR"
    );
  };
  ...
  const joeCost = () => {
    return (
      utils.formatUnits(
        utils.parseUnits(JOE_PRICE, "ether").mul(BigNumber.from(amount)),
        "ether"
      ) + " JOE"
    );
  };
  ...
  const preCheck = () => {
    ...
    if (utils.parseUnits("" + totalWoolCost(), "gwei").gt(woolBalance))
